...
units = st.sidebar.radio(
    ...
    index=0,
    help="(Placeholder) You can extend this later to include kip/ft/ksf.",
)
...
FS_slide_req = st.sidebar.number_input("Required FS (Sliding)", value=default_FS_slide, min_value=1.0, step=0.1, key="FS_slide_req")
FS_ot_req = st.sidebar.number_input("Required FS (Overturning)", value=default_FS_ot, min_value=1.0, step=0.1, key="FS_ot_req")
settle_limit = st.sidebar.number_input("Settlement limit (mm)", value=default_settle_limit_mm, min_value=1.0, step=1.0, key="settle_limit")
...
if st.session_state.page == "home":
    ...
    cards = [
        ...
        ("🎲", "Full Foundation Design (Monte Carlo Robustness)", "Robustness score under uncertainty."),
    ]
    ...
    with top_r:
        st.write("")
        st.button("⬅ Return to homepage", on_click=go_home, kwargs={"clear_inputs": False})
    ...
    if feature == "Bearing Capacity":
        ...
        with tab_in:
            ...
            run = st.button("Run Bearing Capacity", type="primary")
            if run:
    ...
    # -----------------------------------------------------
        ...
        with tab_in:
            run = st.button("Run Settlement", type="primary")
            if run:
    ...
    # -----------------------------------------------------
        ...
        with tab_in:
            run = st.button("Run Sliding Check", type="primary")
            if run:
    ...
    # -----------------------------------------------------
        ...
        with tab_in:
            run = st.button("Run Overturning Check", type="primary")
            if run:
                    ...
                    with st.spinner("Running simulation..."):
                        for i, B in enumerate(B_options):
                            ...
                            for _ in range(iterations):
                                ...
                                if (q_allow >= q_applied) and (FSs >= FS_slide_req) and (FSo >= FS_ot_req):
                                    safe_count += 1

                            robustness = safe_count / iterations
                            results.append((B, robustness))
                            progress.progress(int(((i + 1) / len(B_options)) * 100))
